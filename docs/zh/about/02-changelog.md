...
- **独立部署的 VikingBot Gateway**：`server.bot_api_url` 现在自身即可启用 Bot API 代理，独立运行的
  ...
  `server.bot_gateway_token` 设为网关的 `bot.gateway.token` 即可启用 Web Studio 的机器人管理，同时要求
  两者在同一台主机（管理通道只接受 loopback）。`/api/v1/admin/bot/capabilities` 新增 `mode`
  字段（`managed`/`external`/`disabled`），外部模式下 `enabled` 为 `true`。
