...
- 受管启动时自动为父子进程生成内部管理令牌，不写回配置文件。独立部署的 Gateway 通过 `server.bot_api_url` + `server.bot_gateway_token` 接入：服务端只做代理，不再要求 `--with-bot`；管理通道仍只接受 loopback 请求，因此网关需与 OpenViking Server 同主机。
